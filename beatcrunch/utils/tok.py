# -*- coding: utf-8 -*-
# Based on Natural Language Toolkit: Python port of the tok-tok.pl tokenizer. Copyright (C) 2001-2015 NLTK Project
# Author: G. Fabre - june 2017
# Contributors: Jon Dehdari, Liling Tan, Selcuk Ayguney, ikegami, Martijn Pieters
...
class ToktokTokenizer(TokenizerI):
    ...
    FUNKY_PUNCT_0 = re.compile(u'([\'…\.,:;?!"$%()\[\]«»’‘+/```“#_&])'), r" "

    TOKENIZE_REGEXES = [NON_BREAKING, APOS, FUNKY_PUNCT_1,
                      URL_FOE_1, URL_FOE_2, URL_FOE_3, URL_FOE_4,
                      AMPERCENT, TAB, PIPE,
                      OPEN_PUNCT_RE, CLOSE_PUNCT_RE,
                      MULTI_COMMAS, COMMA_IN_NUM,
                      PROB_SINGLE_QUOTES, STUPID_QUOTES_1, STUPID_QUOTES_2,
                      CURRENCY_SYM_RE, EN_EM_DASHES, MULTI_DASHES, MULTI_DOTS,
                      FINAL_PERIOD_1,FINAL_PERIOD_2, DASH,  FUNKY_PUNCT_0, ONE_SPACE]

    SANITIZE_REGEXES = [DASH, APOS, ONE_SPACE]

    def sanitizeText(self, text):
        text = text_type(text)
        for regexp, subsitution in self.SANITIZE_REGEXES:
            text = regexp.sub(subsitution, text)
        return text

    def tokenize(self, text, return_str=False):
        text = text_type(text) # Converts input string into unicode.
        for regexp, subsitution in self.TOKENIZE_REGEXES:
            text = regexp.sub(subsitution, text)
